...
<c:set var="isCreate" value="${category.idCategoria == 0}"/>
...
<form method="post"action="/TechAddicted/categories/${isCreate ? 'create': 'update'}">
    <c:if test="${not isCreate}">
        <input type="hidden" name="id" value="${category.idCategoria}">
    </c:if>
    <fieldset class="grid-x cell category-form">
        ...
        <label for="label" class="field cell w 40">
            <input id="label" name="label" placeholder="Nome" type="text" value="${category.nomeCategoria}">
        </label>
